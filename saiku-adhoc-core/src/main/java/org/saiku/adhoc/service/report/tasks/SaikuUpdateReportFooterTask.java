...
public class SaikuUpdateReportFooterTask implements UpdateTask {

	private Log log = LogFactory.getLog(SaikuUpdateReportFooterTask.class);
	private List<SaikuLabel> messages;
	private String prefix;
	private SaikuMasterModel model;

	public SaikuUpdateReportFooterTask(List<SaikuLabel> messages,
			String prefix, SaikuMasterModel model) {

		this.messages = messages;
		this.prefix = "rpt-rft-0-";
		this.model = model;
	...
	public void processElement(ReportElement e, int index) {
		...
		if(Boolean.TRUE.equals(el.getParent().
				getAttribute("http://reporting.pentaho.org/namespaces/engine/attributes/wizard", "generated-content-marker"))){
			return;
		}
