...
public class StandalonePrptProvider implements IPrptProvider {
    ...
    public StandalonePrptProvider() {

    }

    public StandalonePrptProvider(String path) {
        try {
            setPath(path);
        } catch (Exception e) {
            e.printStackTrace();
...
//    	}else{
	...
	public MasterReport getPrptTemplate(ReportTemplate reportTemplate) {
		try {
			String path = reportTemplate.getPath() +  reportTemplate.getName() + ".prpt";
			return getMasterReport(path, new SaikuReportingComponent());
